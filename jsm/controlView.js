...
class ControlView  extends THREE.EventDispatcher{
    ...
    constructor(domElement, arg) {
        ...
        this.exportTfFile = function () {
            var output = '';
            var digits = 6;

            var getRowFormat = (a, b, c, d) => {
                return `${a.toFixed(0)} ${b.toFixed(digits)} ${c.toFixed(digits)} ${d.toFixed(digits)}`
            }

            for (let i = 0; i < 256; i++) {
                var row = getRowFormat(arg.rgba[3][i] * 180, arg.rgba[0][i], arg.rgba[1][i], arg.rgba[2][i]);
                output += `${row}\r\n`;
            }

            arg.mylist.forEach((t) => {
                var row = getRowFormat(t.x, t.hsv.H, t.hsv.S, t.hsv.V);
                output += `${row}\r\n`;
            });
        ...
        request.open('GET', "./resources/tf/test2.tf", true)
